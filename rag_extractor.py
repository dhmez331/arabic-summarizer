...
def retrieve_date_chunks(vector_store):
    queries = [
        "تاريخ هجري بالأرقام",
...
def run_evaluation(text, model_key="minilm"):
    # مسار RAG
    vector_store  = build_rag(text, model_key)
    rag_chunks    = retrieve_date_chunks(vector_store)
